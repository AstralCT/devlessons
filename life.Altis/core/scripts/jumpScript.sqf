...
SL_fn_jumpOver = {
	...
	_handled = false;
	if ((_keyCode isEqualTo 57 && _isShift) && (animationState player != SL_jumpAnimation)) then {
...
ripAmmo = {
	params ["_displayCode","_keyCode","_isShift","_isCtrl","_isAlt"];
	_handled = false;
	if ((_keyCode isEqualTo 57 && _isShift) && (animationState player == "ainvpknlmstpslaywrfldnon_medic")) then {
		player setAmmo [currentWeapon player, 0];
		if (isplayer player) then {
		hint "Do not exploit heal!";
		};
	};
};

waituntil {!(isNull (findDisplay 46))};
(findDisplay 46) displayAddEventHandler ["KeyDown", "_this call ripAmmo;"]